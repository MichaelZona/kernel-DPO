...
from trainers.rlchf_trainer import RLCHFTrainer
...
def load_model_with_group_scoring(tokenizer, args):
    """
    Load model and wrap it with group-conditional scoring.
    """
    from transformers import AutoModelForCausalLM
    from peft import get_peft_model, LoraConfig
    
    # Load base pretrained model directly (not with ValueHead)
    ...
    # Wrap with group-conditional scoring (instead of ValueHead)
    model = AutoModelForCausalLMWithGroupConditionalScoring(
        pretrained_model=policy,
        num_groups=getattr(args, 'num_latent_groups', 4),
        use_group_embeddings=getattr(args, 'use_group_embeddings', False),
        group_embedding_dim=getattr(args, 'group_embedding_dim', 64),
        gating_type=getattr(args, 'gating_type', 'prompt_dependent'),
...
def main(args):
    ...
    config = RLCHFConfig(
        ...
        num_latent_groups=getattr(args, 'num_latent_groups', 4),
        aggregation_type=getattr(args, 'aggregation_type', 'weighted_avg'),
        aggregation_temperature=getattr(args, 'aggregation_temperature', 1.0),
        gating_type=getattr(args, 'gating_type', 'prompt_dependent'),
        gating_hidden_size=getattr(args, 'gating_hidden_size', 128),
        entropy_reg_coef=getattr(args, 'entropy_reg_coef', 0.1),
        use_group_embeddings=getattr(args, 'use_group_embeddings', False),
        group_embedding_dim=getattr(args, 'group_embedding_dim', 64),
        project_kwargs={
    ...
    model = load_model_with_group_scoring(tokenizer, args)
    print(args.pretrained_dir)

    trainer = RLCHFTrainer(
        model=model,
        config=config,
        tokenizer=tokenizer,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wandb_project', type=str, default='rlchf', help='the wandb project name')
    parser.add_argument('--run_name', type=str, default='rlchf', help='the wandb run name')
    ...
    # RLCHF-specific arguments
    parser.add_argument('--num_latent_groups', type=int, default=4, help='Number of latent groups K (typically 4-16)')
    parser.add_argument('--aggregation_type', type=str, default='weighted_avg', choices=['weighted_avg', 'log_sum_exp'], help='Aggregation type: weighted_avg or log_sum_exp')
    parser.add_argument('--aggregation_temperature', type=float, default=1.0, help='Temperature τ for log-sum-exp aggregation')
    parser.add_argument('--gating_type', type=str, default='prompt_dependent', choices=['global', 'prompt_dependent'], help='Gating type: global or prompt_dependent')
    parser.add_argument('--gating_hidden_size', type=int, default=128, help='Hidden size for prompt-dependent gating network')
    parser.add_argument('--entropy_reg_coef', type=float, default=0.1, help='Entropy regularization coefficient to prevent collapse')
    parser.add_argument('--use_group_embeddings', type=bool, default=False, help='Use group embeddings instead of K separate heads')
    parser.add_argument('--group_embedding_dim', type=int, default=64, help='Dimension of group embedding')
