...
class SimPOTrainer():
    """
    SimPO (Simplified Preference Optimization) Trainer.
    ...
    2. Length-normalized log probabilities
    3. Additional gamma term
    """
    ...
    def __init__(
        ...
        if self.is_encoder_decoder: 
            raise ValueError("SimPO does not support encoder-decoder models.")
        ...
        current_config = dict(trl_simpo_trainer_config=config.to_dict()) if not is_using_tensorboard else config.to_dict()
        ...
        # SimPO-specific: gamma parameter (default 0 if not specified)
        self.gamma = getattr(self.config, "gamma", 0.0)
    ...
    def _step(
        ...
        """
        SimPO loss computation.
        
        According to SimPO paper (Equation 6):
        L_SimPO = -log σ((β/|y_w|) log π_θ(y_w|x) - (β/|y_l|) log π_θ(y_l|x) - γ)
        
        Where:
        - β (beta) is the temperature parameter
        - |y_w| and |y_l| are the lengths of winner and loser responses
        - log π_θ(y_w|x) and log π_θ(y_l|x) are the log probabilities of responses given prompt
        - γ (gamma) is a constant offset term
        """
        ...
        # Compute log π_θ(y_w|x) and log π_θ(y_l|x) - sum over response tokens only
        # mask_w and mask_l are 1 only for response tokens (prompt tokens are masked to 0)
        seq_logprob_w = (logprobs_w * mask_w).sum(dim=1)          # [B] log π_θ(y_w|x)
        seq_logprob_l = (logprobs_l * mask_l).sum(dim=1)          # [B] log π_θ(y_l|x)
        
        # Compute response lengths |y_w| and |y_l| (number of non-padding tokens in response)
        response_len_w = mask_w.sum(dim=1).float()  # [B] |y_w|
        response_len_l = mask_l.sum(dim=1).float()  # [B] |y_l|
        
        # Avoid division by zero (add small epsilon)
        eps = 1e-8
        response_len_w = response_len_w.clamp(min=eps)
        response_len_l = response_len_l.clamp(min=eps)
        
        # SimPO objective according to paper:
        # L_SimPO = -log σ((β/|y_w|) log π_θ(y_w|x) - (β/|y_l|) log π_θ(y_l|x) - γ)
        beta = self.config.temperature  # β parameter
        simpo_logit = (beta / response_len_w) * seq_logprob_w - (beta / response_len_l) * seq_logprob_l - self.gamma  # [B]
        
        # Loss: -log σ(simpo_logit)
        simpo_loss_vec = -F.logsigmoid(simpo_logit)  # [B]

        denom = sample_mask.sum()
        if denom.item() == 0:
            simpo_loss = simpo_loss_vec.mean()
        else:
            simpo_loss = (simpo_loss_vec * sample_mask).sum() / denom

        # For stats: compute reward margin (for compatibility with evaluation)
        # In SimPO, we don't have reference model, so we use a simple margin
        reward_margin = simpo_logit.detach()  # [B]

        stats = dict(
            loss=dict(
                simpo_loss=simpo_loss.detach(),
            ),
            policy=dict(
                logprobs_w=seq_logprob_w.mean().detach(),
                logprobs_l=seq_logprob_l.mean().detach(),
                norm_logprobs_w=(seq_logprob_w / response_len_w).mean().detach(),
                norm_logprobs_l=(seq_logprob_l / response_len_l).mean().detach(),
                response_len_w=response_len_w.mean().detach(),
                response_len_l=response_len_l.mean().detach(),
                simpo_logit_mean=simpo_logit.mean().detach(),
                classifier_accuracy=(reward_margin > 0).float().mean().detach(),
            ),
        )
        return simpo_loss, flatten_dict(stats)
