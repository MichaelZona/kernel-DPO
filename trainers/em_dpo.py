...
from trainers.em_dpo_trainer import EMDPOTrainer, EMDPOConfig, EMRewardLearner
...
def main(args):
    ...
    # Use EMDPOConfig instead of DPOConfig
    config = EMDPOConfig(
    ...
    # Step 2: Train with standard DPO (using EM-learned clusters and reward models)
    print("\n=== Step 2: Training with Standard DPO ===")
    ...
    def process_pref_batch(pref_batch):
        ...
        # Extract user_ids for cluster-conditioned DPO
    ...
    print("Starting standard DPO training (with EM-learned clusters)")
    total_iterations = 0
    ...
    for epoch in tqdm(range(args.num_train_epochs), desc="Epochs"):
        for sub_iteration, pref_batch in tqdm(enumerate(pref_dataset_dataloader), desc="Batches", total=len(pref_dataset_dataloader)):
            ...
            # Cluster-conditioned DPO step: uses cluster-specific reward models
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wandb_project', type=str, default='em_dpo', help='the wandb project name')
    parser.add_argument('--run_name', type=str, default='em_dpo', help='the wandb run name')
    ...
    # EM-DPO specific arguments
    parser.add_argument('--num_clusters', type=int, default=2, help='number of user clusters for EM algorithm')
